...
body('newPassword','New password is required and must be atleast 8 character long').exists().isString().isLength({min:8}),
async (req,res)=>{
    let err = validationResult(req);
    if(!err.isEmpty()){
        res.status(400).json({
...
body('contactNumber',"Contact Number is a required field").isString().exists().isLength({min:10,max:10}),
async (req,res)=>{
    let err = validationResult(req);
    if(!err.isEmpty()){
        res.status(400).json({
        ...
        try{
            ...
            console.log("Some error occured in admin routes while creating editors")
            console.log(error);
            res.status(500).json({
                "status":false,
